...
int main(int argc, char *argv[]){
	...
	long load 			= atoi(argv[4]); // for large numbers long must be used instead of int
	...
	*/
	...
	printf("%s %s %3d %10ld %9ld\n", programName, label, 0, tP0.tv_sec, tP0.tv_nsec);
	...
	for (int slice = 1; slice <= num_slices; slice++){
		...
		// Now print the output as requested in the exercise.
		printf("%s %s %4d %10ld %9ld\n", programName, label, slice, tPC.tv_sec, tPC.tv_nsec);
	}
